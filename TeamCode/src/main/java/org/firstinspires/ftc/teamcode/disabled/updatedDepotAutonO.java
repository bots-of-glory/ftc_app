...
@Autonomous(name="updatedDepotAutonO", group="Competition Autonomous")
@Disabled
...
//-----------------------------Strafe Towards Wall Start--------------------------------------------
        ...
        frontLeft.setMode(DcMotor.RunMode.RUN_TO_POSITION);
        frontRight.setMode(DcMotor.RunMode.RUN_TO_POSITION);
        rearLeft.setTargetPosition(-825);
        rearLeft.setPower(.5);
        rearRight.setTargetPosition(825);
        rearRight.setPower(.5);
        frontLeft.setTargetPosition(825);
        frontLeft.setPower(.5);
        frontRight.setTargetPosition(-825);
        frontRight.setPower(.5);
        while (rearLeft.isBusy() && opModeIsActive()) {
        }
        while (rearRight.isBusy() && opModeIsActive()) {
